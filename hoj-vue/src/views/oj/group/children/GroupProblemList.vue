<template>
  <el-card>
      ...
      <el-row>
        ...
        <el-col :md="18" :xs="19" v-if="isSuperAdmin || isGroupAdmin">
          ...
          <el-button
            ...
            type="warning"
            size="small"
            @click="handleUsePublicProblem"
            icon="el-icon-folder-add"
            >{{ $t('m.Use_Public_Problem') }}</el-button
          >
        </el-col>
      </el-row>
    </div>
    ...
    <!-- 使用公共题库题目对话框 -->
    <el-dialog
      :title="$t('m.Use_Public_Problem')"
      :visible.sync="usePublicProblemDialogVisible"
      width="80%"
      :close-on-click-modal="false"
    >
      <AddGroupPublicProblem
        ref="addGroupPublicProblem"
        :groupId="$route.params.groupID"
        @currentChangeProblem="handlePublicProblemAdded"
      />
    </el-dialog>
...
import AddGroupPublicProblem from '@/components/oj/group/AddGroupPublicProblem';
export default {
  ...
  data() {
    return {
      ...
      createPage: false,
      editPage: false,
      usePublicProblemDialogVisible: false,
  ...
  methods: {
    ...
    handleUsePublicProblem() {
      this.usePublicProblemDialogVisible = true;
    },
    handlePublicProblemAdded() {
      this.usePublicProblemDialogVisible = false;
      this.init();
    },
