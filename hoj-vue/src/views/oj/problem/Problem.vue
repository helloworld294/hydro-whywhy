...
export default {
  ...
  methods: {
    ...
    // 洛谷风格的庆祝效果
    showLuoguCelebration() {
      // 创建庆祝容器
      const celebrationContainer = document.createElement('div');
      celebrationContainer.className = 'luogu-celebration';
      ...
      // 创建洛谷风格的庆祝图片
      this.createLuoguCelebrationImage(celebrationContainer);
    ...
    createLuoguCelebrationImage(container) {
      // 使用真实的洛谷庆祝图片
      const celebrationImage = document.createElement('img');
      celebrationImage.src = require('@/assets/ac.png');
      celebrationImage.style.cssText = `
        ...
        max-height: 90vh;
        object-fit: contain;
        animation: luogu-celebration-popup 0.8s cubic-bezier(0.68, -0.55, 0.265, 1.55);
      `;

      container.appendChild(celebrationImage);

      // 添加CSS动画样式
      if (!document.getElementById('luogu-celebration-styles')) {
        const style = document.createElement('style');
        style.id = 'luogu-celebration-styles';
        style.textContent = `
          @keyframes luogu-celebration-popup {
            0% {
