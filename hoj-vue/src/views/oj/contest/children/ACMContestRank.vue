  ...
  <el-card shadow>
    <div slot="header">
      <div style="display: flex; justify-content: space-between; align-items: center;">
        ...
        <div v-if="contest.allowEndSubmit" style="display: flex; align-items: center; gap: 10px;">
          <el-radio-group v-model="rankType" @change="onRankTypeChange" size="small">
            <el-radio-button label="contest">{{ $t('m.Contest_Rank') }}</el-radio-button>
            <el-radio-button label="after">{{ $t('m.Contest_After_Rank') }}</el-radio-button>
          </el-radio-group>
        </div>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      dataRank: [],
      keyword: null,
      rankType: 'contest', // 'contest' 或 'after'
      options: {
        title: {
  ...
  methods: {
    ...
    downloadRankCSV() {
      utils.downloadFile(
        ...
        }&forceRefresh=${this.forceUpdate ? true : false}&containEnd=${this.rankType === 'after'}`
      );
    },
    onRankTypeChange() {
      this.getContestRankData(1);
