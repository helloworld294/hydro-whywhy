...
public class ImageManager {
    ...
    public Map<Object, Object> uploadAvatar(MultipartFile image) throws StatusFailException, StatusSystemErrorException {
        ...
        if (accountProfile == null || accountProfile.getUid() == null) {
            throw new StatusFailException("用户未登录或登录信息异常！");
        }

        // 检查用户是否有禁止修改个人资料的权限
        List<Role> roles = userRoleEntityService.getRolesByUid(accountProfile.getUid());
        if (roles != null && !roles.isEmpty()) {
            for (Role role : roles) {
                // 1009: 禁止修改个人资料, 1010: 禁止修改个人资料&禁言
                if (role != null && role.getId() != null) {
                    Long roleId = role.getId();
                    if (roleId.equals(1009L) || roleId.equals(1010L)) {
