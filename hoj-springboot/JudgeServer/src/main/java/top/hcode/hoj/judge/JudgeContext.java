...
public class JudgeContext {
    ...
    public void updateOtherTable(Long submitId,
        ...
        if (cid == 0) { // 非比赛提交
            boolean isTrainingSubmission = false;
            if (gid != null) {
                QueryWrapper<TrainingRecord> queryWrapper = new QueryWrapper<>();
                queryWrapper.select("id").eq("submit_id", submitId);
                isTrainingSubmission = trainingRecordEntityService.count(queryWrapper) > 0;
            }
            boolean shouldRecord = gid == null || isTrainingSubmission;
            if (status.intValue() == Constants.Judge.STATUS_ACCEPTED.getStatus() && shouldRecord) {
